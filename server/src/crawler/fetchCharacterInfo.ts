...
export const fetchCharacterInfo = async (nickname: string, renew: boolean): Promise<CharacterInfo | undefined> => {
    if (!renew && cache.get(nickname)) return cache.get(nickname);
    let characterInfo: CharacterInfo;

    try {
        characterInfo = extractCharacterInfoFromHTML(nickname, (await axios.get(createUrl(nickname))).data);
    } catch (err) {
        characterInfo = extractCharacterInfoFromHTML(nickname, (await axios.get(createUrl(nickname, true))).data);
    }
...
const extractCharacterInfoFromHTML = (nickname: string, html: string) => {
    ...
    const job = detail.children("dl").children("dd").text().replaceAll(" ", "").split("/")[1];
    const image = detail.children(".char_img").children("img")[0].attribs.src;
    const level = Number($tdList.children("td").first().next().next().text().split(".")[1]);
